...
@Repository
public class CreditTransactionDaoImpl extends JdbcDaoSupport implements CreditTransactionDao {
	...
	public void getAccountBalance(AccountBal bal, int acctId) {
		List<CreditTransaction> transactions = findTransactionsByAcctId(acctId);
		if (CollectionUtils.isEmpty(transactions)) {
			bal.setBalanceAmt(BigDecimal.ZERO);
		} else {
			Util.updateCreditBalance(transactions);
			bal.setBalanceAmt(transactions.get(0).getBalanceAmt());			
		}
		bal.setAccountValue(bal.getBalanceAmt());
	}

	private class BaseQuery extends MappingSqlQuery<CreditTransaction> {
	...
	private class Insert extends SqlUpdate {
		private static final String SQL = "insert into card_trans (acct_id, trans_date, description, trans_amt, trans_acct) "
	...
	private class Delete extends SqlUpdate {
		private static final String SQL = "delete from card_trans where trans_id = :transId";
